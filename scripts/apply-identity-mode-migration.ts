...
async function run() {
  console.log('=== APPLYING LAM PRODUCTS IDENTITY MODE MIGRATION ===')
  ...
  try {
    ...
    const sqlPath = path.join(process.cwd(), 'supabase/migrations/20260819000002_lam_products_identity_mode.sql')
    ...
    const { data: prods } = await supabase.from('lam_products').select('slug, name, workspace_prefix, identity_mode, client_id, app_url')
    console.log('\n--- VERIFIED LAM PRODUCTS REGISTRY ---')
    console.table(prods)
